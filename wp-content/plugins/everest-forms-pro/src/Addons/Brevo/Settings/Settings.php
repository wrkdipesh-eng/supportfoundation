<?php
/**
 * EverestForms Sendinblue Admin.
 *
 * @package EverestForms\Pro\Addons\Settings\Settings
 * @since   1.7.7
 */

namespace EverestForms\Pro\Addons\Brevo\Settings;
...
class Settings extends \EVF_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = esc_html__( 'Brevo', 'everest-forms-pro' );
		$this->method_description = esc_html__( 'Brevo Integration with Everest Forms', 'everest-forms-pro' );
		...
		add_action( 'everest_forms_integration_account_connect_sendinblue', array( $this, 'api_connect' ) );
	}
	...
	/**
	 * API Connect Sendinblue function.
	 *
	...
	public function api_connect( $posted_data ) {
		...
		if ( empty( $posted_data['apikey'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'Brevo API key is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		$api   = new API( trim( $posted_data['apikey'] ) );
		$valid = $api->validate_api_key();
		...
		$connected_accounts['sendinblue'][ $to_be_stored_id ] = array(
	...
	public function output_connection_form() {
		...
		<div class="integration-connection-detail">
			<div class="evf-account-connect">
				...
				<p><?php esc_html_e( 'Fill out the fields to connect your Brevo account', 'everest-forms-pro' ); ?></p>
				<form>
					<div class="evf-connection-form">
						<div class="evf-connection-field">
							<label>
								<strong><?php esc_html_e( 'Brevo API Key', 'everest-forms-pro' ); ?></strong>
								<input type="text" placeholder="<?php esc_attr_e( 'Enter Brevo API Key', 'everest-forms-pro' ); ?>" class="evf-apikey">
							</label>
						</div>
						<div class="evf-connection-field">
							<label>
					...
					<a href="#" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-connect-account" data-source="<?php echo esc_attr( $this->id ); ?>">
						<?php esc_html_e( 'Connect to Brevo', 'everest-forms-pro' ); ?>
					</a>
				</form>
			</div>
			<div class="evf-connection-list">
				<table class="evf-connection-list-table">
					<tbody>
						<?php
						if ( ! empty( $connected_lists['sendinblue'] ) ) {
							foreach ( $connected_lists['sendinblue'] as $key => $list ) {
								?>
								<tr>
									<td><strong><?php echo esc_html( $list['label'] ); ?></strong></td>
									...
									<td>
										<a href='#' class='disconnect everest-forms-integration-disconnect-account' data-source='sendinblue' data-key='<?php echo esc_attr( $key ); ?>'>
	...
	/**
	 * Facilitates sendinblue integration. Evoked from extensibility.
	 */
