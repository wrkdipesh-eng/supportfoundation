 ...
 *
 * @package EverestForms\Pro\Addons\CleverReach\Builder
 * @since   3.0.5
 */

namespace  EverestForms\Pro\Addons\CleverReach\Builder;

use EverestForms\Pro\Addons\CleverReach\API\API;
use EverestForms\Pro\Addons\CleverReach\Settings\Settings as CleverReachIntegrations;
...
class Builder extends  \EVF_Email_Marketing {
	...
	public function __construct() {
		$this->id   = 'cleverreach';
		$this->name = __( 'CleverReach', 'everest-forms-pro' );
		$this->icon = plugins_url( 'src/Addons/CleverReach/assets/images/cleverreach.png', EFP_PLUGIN_FILE );

		parent::__construct();
		add_filter( 'everest_forms_cleverreach_add_account_html', array( $this, 'outputs_authentication_html' ) );
	}
			...
			if ( ! is_wp_error( $this->api ) ) {
				try {
					...
					if ( isset( $custom_field_value ) && ! empty( $custom_field_value ) ) {
						$data['global_attributes'] = $custom_field_value;
					}
					...
					if ( isset( $response['error'] ) ) {
						...
						$logger->error(
							sprintf(
								esc_html__( 'CleverReach Error: %s', 'everest-forms-pro' ),
					...
					$logger = evf_get_logger();
					$logger->log(
						sprintf( esc_html__( 'CleverReach error: ', 'everest-forms-pro' ) . $ex->getMessage() ),
	 ...
	 * Connect to API.
	 *
	 * @param  string $account_id  Account ID for CleverReach Contact.
	...
	public function api_lists( $connection_id = '', $account_id = '' ) {
		...
		try {
			...
			evf_get_logger(
				__( 'CleverReach API error', 'everest-forms-pro' ),
				$e->getMessage(),
				array(
	...
	public function api_groups( $connection_id = '', $account_id = '', $list_id = '' ) {
		return $this->error( esc_html__( 'CleverReach won\'t support Groups.', 'everest-forms-pro' ) );
	}
	...
	public function api_custom_field( $connection_id = '', $account_id = '', $list_id = '' ) {
		...
		try {
			...
			if ( ! is_wp_error( $lists ) && ! empty( $lists ) ) {
				...
				$exclude_fields = apply_filters( 'everest_forms_exclude_fields_in_cleverreach_list', array( 'email', 'firstname', 'lastname', 'phone' ) );
			...
			evf_get_logger(
				__( 'CleverReach API field error', 'everest-forms-pro' ),
				$e->getMessage(),
				array(
					'type'   => array( 'Integration', 'error' ),
					'source' => 'cleverreach'
	...
	public function outputs_authentication_html( $output ) {
		ob_start();
		?>
		<button type="submit" class="everest-forms-hidden everest-forms-btn everest-forms-btn-primary " data-source="<?php echo esc_attr( $this->id ); ?>"><?php esc_html_e( 'Connect to CleverReach', 'everest-forms-pro' ); ?></button>
		<a href="<?php echo esc_url( filter_var( CleverReachIntegrations::create_auth_url(), FILTER_SANITIZE_URL ) ); ?>"" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-open-window" data-source="<?php echo esc_attr( $this->id ); ?>">
