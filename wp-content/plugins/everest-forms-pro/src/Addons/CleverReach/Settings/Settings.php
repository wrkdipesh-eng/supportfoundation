 ...
 *
 * @package EverestForms\Pro\Addons\CleverReach\Settings
 * @since   3.0.5
 */

namespace EverestForms\Pro\Addons\CleverReach\Settings;

use EverestForms\Pro\Addons\CleverReach\API\API;
...
class Settings extends \EVF_Integration {
	...
	public function __construct() {
		$this->id                 = 'cleverreach';
		$this->icon               = plugins_url( 'src/Addons/CleverReach/assets/images/cleverreach.png', EFP_PLUGIN_FILE );
		$this->method_title       = esc_html__( 'CleverReach', 'everest-forms-pro' );
		$this->method_description = esc_html__( 'CleverReach Integration with Everest Forms', 'everest-forms-pro' );
		...
		add_action( 'everest_forms_integration_account_connect_cleverreach', array( $this, 'api_connect' ) );
		add_action( 'admin_init', array( $this, 'cleverreach_token_page' ) );
	}
	...
	public function admin_enqueue_scripts() {
		...
		if ( 'everest-forms_page_evf-settings' === $screen_id || 'everest-forms_page_evf-builder' === $screen_id ) {
			wp_enqueue_style( 'everest-forms-cleverreach-style', plugins_url( 'src/Addons/CleverReach/assets/css/admin.css', EFP_PLUGIN_FILE ), array(), EFP_VERSION );
		}
	}

	/**
	 * API Connect CleverReach function.
	 *
	...
	public function api_connect( $posted_data ) {
		...
		if ( empty( $posted_data['client_id'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'Invalid CleverReach Client ID.', 'everest-forms-pro' ),
				)
			);
		}
		...
		if ( empty( $posted_data['client_secret'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'CleverReach Client Secret is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		if ( empty( $posted_data['auth_code'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'CleverReach Access Code is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		if ( empty( $posted_data['label'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'CleverReach account nickname is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		try {
			...
			$settings = $client->create_access_token( $posted_data['auth_code'], $connection );
			...
			if ( isset( $settings['access_token'] ) && ! empty( $settings['access_token'] ) ) {
				...
				$connected_accounts['cleverreach'][ $api_key_to_be_connected ] = $settings;
				...
				if ( isset( $settings['error_description'] ) ) {
					$error_message .= "`{$settings['error_description']}` for CleverReach";
				}
	...
	<div class="integration-connection-detail">
		<div class="evf-account-connect">
			...
			<p>
				...
				printf(
					...
					esc_html__(
						'Get the CleverReach access code, we will use that code to authenticate with CleverReach. Follow the %1$s documentation %2$s to get CleverReach Client ID and Secret Key.',
						'everest-forms-pro'
					),
					'<a href="https://docs.everestforms.net/docs/clever-reach/" target="_blank">',
					'</a>'
				);
				?>
			</p>
			<form>
				<div class="evf-connection-form cleverreach-connection-form">
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'CleverReach Client ID', 'everest-forms-pro' ); ?></strong>
							<input type="text" name="client_id" placeholder="<?php esc_attr_e( 'Enter CleverReach Client ID', 'everest-forms-pro' ); ?>" class="evf_cr_client_id" required>
						</label>
					</div>
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'CleverReach Client Secret', 'everest-forms-pro' ); ?></strong>
							<input type="password" name="client_secret" placeholder="<?php esc_attr_e( 'Enter CleverReach Client Secret', 'everest-forms-pro' ); ?>" class="evf_cr_client_secret" required>
						</label>
					</div>
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'CleverReach Account Nickname', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter Account Nickname', 'everest-forms-pro' ); ?>" class="evf_cr_label" required>
						</label>
					</div>
					<div class="evf-connection-field everest-forms-hidden">
						<label>
							<strong><?php esc_html_e( 'Access Code', 'everest-forms-pro' ); ?></strong>
							<input type="password" name="<?php echo esc_attr( 'access_code' ); ?>" class="evf_cr_access_code" placeholder="<?php esc_attr_e( 'Enter Access Code', 'everest-forms-pro' ); ?>" required>
						</label>
					</div>
				</div>
				<div class="everest-forms-hidden">
					<button type="submit" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-connect-account" data-source="<?php echo esc_attr( $this->id ); ?>">
						<?php esc_html_e( 'Connect to CleverReach', 'everest-forms-pro' ); ?>
					</button>
				</div>
		...
		<div class="evf-connection-list">
			<table class="evf-connection-list-table">
				<tbody>
					<?php
					if ( ! empty( $connected_lists['cleverreach'] ) ) {
						foreach ( $connected_lists['cleverreach'] as $key => $list ) {
							?>
							<tr>
								...
								<td>
									<a href='#' class='disconnect everest-forms-integration-disconnect-account' data-source='cleverreach' data-key='<?php echo esc_attr( $key ); ?>'>
	...
	/**
	 * Facilitates cleverreach integration. Evoked from extensibility.
	 ...
	 * @since 3.0.5
	 */
	public function cleverreach_token_page() {
		if ( isset( $_REQUEST['evf_cleverreach_auth'] ) ) { // phpcs:ignore
			...
			if ( isset( $_REQUEST['code'] ) ) { // phpcs:ignore
				...
				<html lang="en">
					<head>
						...
						<title><?php esc_html( 'CleverReach - Access Code || Everest Forms' ); ?></title>
						<style>
					...
					<body>
						<div class="evf_cleverreach_access_token" id="evf_aweber_access_token" style="width:80%; margin:auto; margin-top: 80px;">
							<p><h3><?php esc_html( 'Your CleverReach Access Code is: ' ); ?></h3></p>
							<br/>
