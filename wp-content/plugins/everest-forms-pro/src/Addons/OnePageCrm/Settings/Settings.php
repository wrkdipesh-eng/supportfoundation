 ...
 * OnePageCRM Settings.
 *
 * @package EverestForms\OnePageCRM\Settings
 * @since   1.0.0
 */

namespace EverestForms\Pro\Addons\OnePageCrm\Settings;
...
class Settings extends \EVF_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = esc_html__( 'OnePageCRM', 'everest-forms-pro' );
		$this->method_description = esc_html__( 'OnePageCRM Integration with Everest Forms', 'everest-forms-pro' );
		...
		add_action( 'everest_forms_integration_account_connect_onepagecrm', array( $this, 'api_connect' ) );
	}
	...
	/**
	 * API Connect OnePageCRM function.
	 *
	...
	public function api_connect( $posted_data ) {
		...
		if ( empty( $posted_data['apikey'] ) && empty( $posted_data['apiuserid'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'OnePageCRM API key and User ID are required.', 'everest-forms-pro' ),
				)
			);
		}

		try {
			$api_key_to_be_connected = new Api( trim( $posted_data['apikey'] ), trim( $posted_data['apiuserid'] ) );
			$valid                   = $api_key_to_be_connected->auth_test();
			if ( isset( $valid['status'] ) && 401 === $valid['status'] ) {
				throw new \Exception( "Invalid OnePageCRM API key `{$posted_data['apikey']}` or invalid user id `{$posted_data['apiuserid']}`supplied." );
			}
		} catch ( \Exception $e ) {
			wp_send_json_error(
				array(
		...
		$connected_accounts['onepagecrm'][ $to_be_stored_id ] = array(
			'apiuserid' => trim( trim( $posted_data['apiuserid'] ) ),
	...
	<div class="integration-connection-detail">
		<div class="evf-account-connect">
			...
			<p><?php esc_html_e( 'Fill out the fields to connect your OnePageCRM account', 'everest-forms-pro' ); ?></p>
			<form>
				<div class="evf-connection-form">
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'OnePageCRM User ID', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter OnePageCRM User ID', 'everest-forms-pro' ); ?>" class="evf-apiuserid">
						</label>
					</div>
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'OnePageCRM API Key', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter OnePageCRM API Key', 'everest-forms-pro' ); ?>" class="evf-apikey">
						</label>
					</div>
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'OnePageCRM Nick Name', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter OnePageCRM Nick Name', 'everest-forms-pro' ); ?>" class="evf-nickname">
						</label>
					</div>
				</div>
				<a href="#" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-connect-account" data-source="<?php echo esc_attr( $this->id ); ?>">
					<?php esc_html_e( 'Connect to OnePageCRM', 'everest-forms-pro' ); ?>
				</a>
			</form>
		</div>
		<div class="evf-connection-list">
			<table class="evf-connection-list-table">
				<tbody>
					<?php
					if ( ! empty( $connected_lists ) && isset( $connected_lists['onepagecrm'] ) ) {
						foreach ( $connected_lists['onepagecrm'] as $key => $list ) {
							?>
							<tr>
								...
								<td>
									<a href='#' class='disconnect everest-forms-integration-disconnect-account' data-source='onepagecrm' data-key='<?php echo esc_attr( $key ); ?>'>
	...
	/**
	 * Facilitates OnePageCRM integration. Evoked from extensibility.
