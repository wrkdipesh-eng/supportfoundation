 ...
 * Moosend Settings.
 *
 * @package EverestForms\Moosend\Settings
 * @since   1.0.0
 */

namespace EverestForms\Pro\Addons\Moosend\Settings;
...
class Settings extends \EVF_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = esc_html__( 'Moosend', 'everest-forms-pro' );
		$this->method_description = esc_html__( 'Moosend Integration with Everest Forms', 'everest-forms-pro' );
		...
		add_action( 'everest_forms_integration_account_connect_moosend', array( $this, 'api_connect' ) );
	}
	...
	public function api_connect( $posted_data ) {
		...
		if ( empty( $posted_data['apikey'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'Moosend API key is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		try {
			...
			if ( isset( $valid['Code'] ) && 104 === $valid['Code'] ) {
				throw new \Exception( "Invalid Moosend API key `{$posted_data['apikey']}` supplied." );
			}
		} catch ( \Exception $e ) {
			wp_send_json_error(
				array(
		...
		$connected_accounts['moosend'][ $to_be_stored_id ] = array(
	...
	<div class="integration-connection-detail">
		<div class="evf-account-connect">
			...
			<p><?php esc_html_e( 'Fill out the fields to connect your Moosend account', 'everest-forms-pro' ); ?></p>
			<form>
				<div class="evf-connection-form">
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'Moosend API Key', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter Moosend API Key', 'everest-forms-pro' ); ?>" class="evf-apikey">
						</label>
					</div>
					<div class="evf-connection-field">
						<label>
							<strong><?php esc_html_e( 'Moosend Nick Name', 'everest-forms-pro' ); ?></strong>
							<input type="text" placeholder="<?php esc_attr_e( 'Enter Moosend Nick Name', 'everest-forms-pro' ); ?>" class="evf-nickname">
						</label>
					</div>
				</div>
				<a href="#" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-connect-account" data-source="<?php echo esc_attr( $this->id ); ?>">
					<?php esc_html_e( 'Connect to Moosend', 'everest-forms-pro' ); ?>
				</a>
			</form>
		</div>
		<div class="evf-connection-list">
			<table class="evf-connection-list-table">
				<tbody>
					<?php
					if ( ! empty( $connected_lists ) && isset( $connected_lists['moosend'] ) ) {
						foreach ( $connected_lists['moosend'] as $key => $list ) {
							?>
							<tr>
								...
								<td>
									<a href='#' class='disconnect everest-forms-integration-disconnect-account' data-source='moosend' data-key='<?php echo esc_attr( $key ); ?>'>
	...
	/**
	 * Facilitates moosend integration. Evoked from extensibility.
