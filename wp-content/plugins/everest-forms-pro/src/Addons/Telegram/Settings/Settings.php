<?php
/**
 * Everest Forms Telegram Settings.
 *
 * @package EverestForms\Pro\Telegram\Settings
 * @since   1.0.0
 */

namespace EverestForms\Pro\Addons\Telegram\Settings;
...
class Settings extends \EVF_Integration {
	...
	public function __construct() {
		$this->id                 = 'telegram';
		$this->icon               = plugins_url( 'src/Addons/Telegram/assets/img/telegram.png', EFP_PLUGIN_FILE );
		$this->method_title       = esc_html__( 'Telegram', 'everest-forms-pro' );
		$this->method_description = esc_html__( 'Telegram Integration with Everest Forms', 'everest-forms-pro' );
		...
		add_action( 'everest_forms_integration_account_connect_telegram', array( $this, 'api_connect' ) );
	}
	...
	public function api_connect( $posted_data ) {
		...
		if ( empty( $posted_data['apikey'] ) ) {
			wp_send_json_error(
				array(
					'error_msg' => esc_html__( 'Telegram API key is required.', 'everest-forms-pro' ),
				)
			);
		}
		...
		try {
			...
			if ( isset( $valid['response']['code'] ) && 404 === $valid['response']['code'] ) {
				throw new \Exception( "Invalid Telegram API key `{$posted_data['apikey']}` supplied." );
			}
		} catch ( \Exception $e ) {
			wp_send_json_error(
				array(
		...
		$connected_accounts['telegram'][ $to_be_stored_id ] = array(
	...
	<div class="integration-connection-detail">
		<div class="evf-account-connect">
			...
			<p><?php esc_html_e( 'Fill out the fields to connect your Telegram account', 'everest-forms-pro' ); ?></p>
			<form>
				...
				<a href="#" class="everest-forms-btn everest-forms-btn-primary everest-forms-integration-connect-account" data-source="<?php echo esc_attr( $this->id ); ?>">
					<?php esc_html_e( 'Connect to Telegram', 'everest-forms-pro' ); ?>
				</a>
			</form>
		</div>
		<div class="evf-connection-list">
			<table class="evf-connection-list-table">
				<tbody>
					<?php
					if ( ! empty( $connected_lists ) && isset( $connected_lists['telegram'] ) ) {
						foreach ( $connected_lists['telegram'] as $key => $list ) {
							?>
							<tr>
								...
								<td>
									<a href='#' class='disconnect everest-forms-integration-disconnect-account' data-source='telegram' data-key='<?php echo esc_attr( $key ); ?>'>
	...
	/**
	 * Facilitates telegram integration. Evoked from extensibility.
