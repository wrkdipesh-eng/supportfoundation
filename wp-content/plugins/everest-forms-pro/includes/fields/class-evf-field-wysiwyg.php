...
class EVF_Field_Wysiwyg extends EVF_Form_Fields {
	...
	/**
	 * Build a valid editor element ID for wp_editor().
	 *
	 * WordPress editor IDs cannot contain square brackets.
	 *
	 * @since 1.8.2.4
	 *
	 * @param array $primary Primary field input config.
	 * @return string
	 */
	protected function get_wp_editor_element_id( $primary ) {
		$editor_id = '';

		if ( ! empty( $primary['attr']['id'] ) ) {
			$editor_id = (string) $primary['attr']['id'];
		} elseif ( ! empty( $primary['attr']['name'] ) ) {
			$editor_id = (string) $primary['attr']['name'];
		}

		$editor_id = str_replace( array( '[', ']' ), '_', $editor_id );
		$editor_id = preg_replace( '/[^A-Za-z0-9_-]/', '_', $editor_id );
		$editor_id = trim( $editor_id, '_' );

		if ( '' === $editor_id ) {
			$editor_id = 'evf-wysiwyg-editor';
		}

		return sanitize_key( $editor_id );
	}
	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		if ( isset( $primary['attr']['value'] ) ) {
			$value = wp_kses_post( wp_unslash( $primary['attr']['value'] ) );
			unset( $primary['attr']['value'] );
		}

		$editor_id = $this->get_wp_editor_element_id( $primary );
		if ( isset( $primary['attr']['id'] ) ) {
			unset( $primary['attr']['id'] );
		}
