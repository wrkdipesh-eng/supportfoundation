...
class EVF_Field_Payment_Checkbox extends EVF_Form_Fields {
	...
	public function html_field_value( $value, $field, $form_data = array(), $context = '' ) {
		if ( is_serialized( $field ) || in_array( $context, array( 'entry-single', 'email-html', 'export-pdf' ), true ) ) {
			$field_value = maybe_unserialize( $field );
			$field_type  = isset( $field_value['type'] ) ? sanitize_text_field( wp_unslash( (string) $field_value['type'] ) ) : 'payment-checkbox';
			...
			if ( $field_type === $this->type ) {
				if (
					...
					return implode( 'export-csv' !== $context ? '<br><br>' : '|', $items );
						...
						&& ! empty( $field_value['label'] )
						&& is_array( $field_value['label'] )
						&& empty( $field_value['images'] )
					) {
						return implode( '<br/>', $field_value['label'] );
					}
			}
		}
	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		foreach ( $choices as $choice ) {
			...
			if ( ! empty( $field['choices_images'] ) ) {
				...
				// Normal display.
				printf( '<input type="checkbox" %s %s %s>', evf_html_attributes( $choice['id'], $choice['class'], $choice['data'], $choice['attr'] ), esc_attr( $choice['required'] ), checked( '1', $choice['default'], false ) );
				printf( '<label %s>%s</label>', evf_html_attributes( $choice['label']['id'], $choice['label']['class'], $choice['label']['data'], $choice['label']['attr'] ), apply_filters( 'payment_checkbox_label_text', wp_kses_post( $choice['label']['text'] ) ) );
			}
