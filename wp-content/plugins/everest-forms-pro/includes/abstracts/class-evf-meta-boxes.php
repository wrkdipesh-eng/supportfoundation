...
/**
 * Abstract EVF_Meta_Boxes Class
 *
 * @since v2.0.0
 * @package  EverestForms/Abstracts
 */
abstract class EVF_Meta_Boxes {
	 ...
	 */
	public function evf_metabox_checkbox( $field ) {
		...
		$field['class']         = isset( $field['class'] ) ? $field['class'] : 'evffl-checkbox';
		...
		echo '<div class="evf-metabox-field ' . esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '">';
		echo '<div class="evf-metabox-field-row">';
		echo '<div class="evf-metabox-field-label">';
		echo '<label for="' . esc_attr( $field['id'] ) . '">' . wp_kses_post( $field['label'] ) . '</label>';
		echo evf_help_tip( $field['desc'] );
		echo '</div>';

		echo '<div class="evf-metabox-field-detail">';
	 ...
	 * @param array $field Metabox Field.
	 */
	public function evf_metabox_select( $field ) {
	 ...
	 * @param array $field Metabox Field.
	 */
	public function evf_metabox_multiple_select( $field ) {
		...
		echo '<div class="evf-metabox-field ' . esc_attr( chop( $field['id'], '[]' ) ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '">';
		echo '<div class="evf-metabox-field-row">';
		echo '<div class="evf-metabox-field-label">';
		echo '<label for="' . esc_attr( $field['id'] ) . '">' . wp_kses_post( $field['label'] ) . '</label>';
		echo  evf_help_tip( $field['desc'] );
	 ...
	 * @param array $field Metabox Field.
	 */
	public function evf_metabox_input( $field ) {
		...
		$thepostid              = empty( $thepostid ) ? $post->ID : $thepostid;
		$field['class']         = isset( $field['class'] ) ? $field['class'] : 'evffl-input';
		$field['style']         = isset( $field['style'] ) ? $field['style'] : '';
		$field['wrapper_class'] = isset( $field['wrapper_class'] ) ? $field['wrapper_class'] : '';
		$field['value'] 		= isset($get_meta_data) && $get_meta_data !== '' ? $get_meta_data : ( isset( $field['value'] ) ? $field['value'] : '' );
		$field['name']          = isset( $field['name'] ) ? $field['name'] : $field['id'];
		$field['desc']          = isset( $field['desc'] ) ? $field['desc'] : '';
